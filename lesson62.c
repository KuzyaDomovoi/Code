...
void show_feld(const char (*f)[N]) // auxiliary func represented as 2D-array to dysplaying the game feld;
{
    for(int i = 0; i < N; ++i) { // iterate the rows of game feld;
        for(int j = 0; j < N; ++j) // terate the cols of game feld;
            printf("%c ", (f[i][j] == 0) ? '#' : '0'); // output sing '#' if current value is '0' (closed) or sign '0' if current value is '1' (opened);
...
void open_zeros(const char (*f)[N], char (*pf)[N], int i, int j) // is rcs func and it opens cells ([i], [j]) of game feld which have value 0;
{
    ...
    if(i + 1 <  N && f[i + 1][j] == 0) open_zeros(f, pf, i + 1, j); // check neighboring cells to 1 cell right direction for value zero and it to open;
    if(j - 1 >= 0 && f[i][j - 1] == 0) open_zeros(f, pf, i, j - 1); // check neighboring cells to 1 cell up direction for value zero and it to open;
    if(j + 1 <  N && f[i][j + 1] == 0) open_zeros(f, pf, i, j + 1); // check neighboring cells to 1 cell down direction for value zero and it to open;
}
